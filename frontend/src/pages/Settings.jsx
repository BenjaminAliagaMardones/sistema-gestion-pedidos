        ...
        <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem', maxWidth: '900px' }}>
                    ...
                    <form onSubmit={handleSave} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                        ...
                        <div className="form-group">
                            ...
                            <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>
                                Esta tasa se usará por defecto al crear pedidos
                            </span>
                        </div>
                ...
                <div className="card">
                    ...
                    {config?.logo_path && (
                        <div style={{ marginBottom: '1rem', padding: '1rem', background: 'var(--bg-elevated)', borderRadius: '8px', textAlign: 'center' }}>
                            <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>Logo actual configurado ✓</span>
                        </div>
                    )}
                ...
                <div className="card" style={{ gridColumn: '1 / -1' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '1rem' }}>
                        {[
                            ...
                            { label: 'Tasa Actual', value: `${config?.default_exchange_rate?.toLocaleString('es-CL')} CLP/USD` },
                            { label: 'Última Actualización', value: config?.updated_at ? new Date(config.updated_at).toLocaleDateString('es-CL') : '—' },
                        ].map((item) => (
                            <div key={item.label} style={{ padding: '0.75rem', background: 'var(--bg-elevated)', borderRadius: '8px' }}>
                                <div style={{ fontSize: '0.7rem', color: 'var(--text-muted)', textTransform: 'uppercase', marginBottom: '4px' }}>{item.label}</div>
